...
- **Hook**: `useZuluConnection` manages `ZwsSession[]` state, builds `EditableLayer`-compatible objects with negative IDs. Saved connections are loaded on init for UI display only — **no in-memory ZWS sessions are auto-created** (prevents N×48 HTTP flood). User must explicitly click "Connect" on a saved connection to create in-memory layer sessions. Schema loading (`fetchZwsLayerSchema`/GetLayerBaseInfo) is **skipped on init**.
...
- **Geometry types**: Full OGC WKT support — Point, LineString, Polygon, MultiPoint, MultiLineString, MultiPolygon, GeometryCollection. Z-coordinates preserved as `[lon, lat, z]` (OpenLayers ignores Z for 2D rendering). `parseWktToGeoJsonCoordinates` in `server/routes.ts` uses bracket-level parsing for MULTIPOLYGON (proper sub-polygon + hole handling). GeometryCollection is decomposed into separate features during import. Layer `geometryType` is set to the dominant type detected across all features. ZWS layers can be heterogeneous (mixed Point+LineString+Polygon); the style function dispatches by actual feature geometry type.
